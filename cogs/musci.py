...
class Music(commands.Cog, discordSuperUtils.CogManager.Cog, name="Music"):
    ...
    async def cog_before_invoke(self, ctx: commands.Context):
        ...
        if ctx.command.name != '메일':
            ...
            if str(check) != str(check2[1]):
                mal = discord.Embed(
                    title=f'📫짱구봇 메일함 | {int(check) - int(check2[1])}개 수신됨',
                    description="아직 읽지 않은 메일이 있어요.'`짱구야 메일`'로 확인하세요.\n주기적으로 메일함을 확인해주세요! 소소한 업데이트 및 이벤트개최등 여러소식을 확인해보세요.",
                    colour=ctx.author.colour,
                )
    ...
    async def loop(self, ctx):
        ...
        if is_loop is not None:
            await ctx.send(
                f"반복기능을 {'활성화 <:activ:896255701641474068>' if is_loop else '비활성화 <:disactiv:896388083816218654>'} 했어요",delete_after=5)
    ...
    async def queueloop(self, ctx):
        ...
        if is_loop is not None:
            await ctx.send(
                f"큐반복기능을 {'활성화 <:activ:896255701641474068>' if is_loop else '비활성화 <:disactiv:896388083816218654>'} 했어요",delete_after=5)
    ...
    async def skip(self, ctx, index: int = None):
        if queue := (await self.MusicManager.get_queue(ctx)):
            ...
            if requester is None:
                await ctx.send("자동재생 곡을 스킵했어요.⏩",delete_after=5)
    ...
    async def loop_check(self, ctx):
        if queue := await self.MusicManager.get_queue(ctx):
            ...
            if loop == discordSuperUtils.Loops.LOOP:
                loop_status = "반복 기능이 활성화 되었어요. <:boton:933726902923984946>"

            elif loop == discordSuperUtils.Loops.QUEUE_LOOP:
                loop_status = "큐 반복 기능이 활성화 되었어요. <:boton:933726902923984946>"

            elif loop == discordSuperUtils.Loops.NO_LOOP:
                loop_status = "반복 기능이 비활성화 되었어요. <:botoff:933726671431950378>"
