...
class twitch(commands.Cog):
    ...
    async def twitch_(self,ctx):
        ...
        if twitch_resp == []:
            return await ctx.reply("등록된 채널이 하나도 없어요! `짱구야 트위치 등록 [맨션될 역할] [알림이 갈 채널] [채널ID]`로 등록하세요!")
        ...
        e = Paginator(
            ...
            embeds=discordSuperUtils.generate_embeds(
                formatted_leaderboard,
                title="트위치 채널 목록 | <:badge:904937799701110814>프리미엄플랜(채널 개수 5개 제한)",
                fields=3,
    ...
    async def twitch_del(self,ctx):
        ...
        twitch_resp = await twitch_cur.fetchall()
        if twitch_resp == []:
            return await ctx.reply("등록된 채널이 하나도 없어요! `짱구야 트위치 등록 [채널ID]`로 등록하세요!")
